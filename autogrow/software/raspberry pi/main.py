...
def choixDesPlantes():
    print(plantechoix)
    donnéesplante = pd.read_csv('autogrow/autogrow/software/raspberry pi/db.csv', usecols= [idPlante-1])
...
plantesdb = pd.read_csv("autogrow/autogrow/software/raspberry pi/db.csv")
i = 1
def quit():
   root.destroy()
root = Tk()
...
x=1
donnéesplante = pd.read_csv('autogrow/autogrow/software/raspberry pi/db.csv', usecols= [idPlante])
