...
def choixDesPlantes():
    print(plantechoix)
    donnéesplante = pd.read_csv('database/db.csv', usecols= [idPlante-1])
...
plantesdb = pd.read_csv('database/db.csv')
i = 1
def quit():
   root.destroy()
root = Tk()
...
x=1
donnéesplante = pd.read_csv('database/db.csv', usecols= [idPlante])
